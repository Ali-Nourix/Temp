...
(function (global) {
  ...
  function svgFrame(seed, opts) {
    ...
    var drift = Math.sin(t * Math.PI * 2) * 12;
    var scale = 1 + Math.sin(t * Math.PI * 2 + 1) * 0.02;
    ...
    for (var i = 0; i < 90; i++) {
      grain += '<circle cx="' + round(gr() * 1000, 1) + '" cy="' + round(gr() * 1250, 1) +
        '" r="' + round(0.6 + gr() * 1.6, 2) + '" fill="#000" opacity="' + round(0.02 + gr() * 0.05, 3) + '"/>';
    }

    return (
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1250" width="' + w + '" height="' + h +
      ...
      '</defs>' +
      '<rect width="1000" height="1250" fill="url(#bg' + id + ')"/>' +
      '<rect width="1000" height="1250" fill="url(#key' + id + ')"/>' +
      /* the surface the object sits on */
      '<rect y="880" width="1000" height="370" fill="' + pal.deep + '" opacity="0.07"/>' +
      '<ellipse cx="500" cy="' + (872 + drift * 0.4) + '" rx="' + round(300 * spinSquash) + '" ry="34" fill="' + pal.deep + '" opacity="0.2"/>' +
      '<g transform="translate(0 ' + round(drift) + ') translate(500 640) scale(' + round(spinSquash * scale, 3) + ' ' + round(scale, 3) + ') translate(-500 -640)">' +
      body + '</g>' +
      '<rect width="1000" height="1250" fill="url(#vig' + id + ')"/>' +
      grain +
      '</svg>'
    );
  }
  ...
  function SyntheticClip(canvas, descriptor) {
    this.canvas = canvas;
  ...
  SyntheticClip.prototype._buildFrames = function () {
    ...
    for (var i = 0; i < n; i++) {
      ...
      img.src = svgUrl(this.d.seed, {
        w: 800, h: 1000, palette: this.d.palette, subject: this.d.subject, t: i / n
      });
  ...
  function mountMedia(host, descriptors, meta, viewer, lang) {
    ...
    var img = el('img', '', { src: d.poster || fullUrl(d), alt: meta.title || '', loading: 'lazy' });
